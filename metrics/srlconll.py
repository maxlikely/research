...
def print_latex(E):
    ...
    p, r, f1 = precrecf1(E['ok'], E['op'], E['ms']) 
    print "%-10s & %6.2f\\%% & %6.2f\\%% & %6.2f\\\\" % ("Overall", p, r, f1)
    print r'\hline'

    for arg_type in sorted(E['T']):
        p, r, f1 = precrecf1(E['T'][arg_type]['ok'], E['T'][arg_type]['op'], \
                                        E['T'][arg_type]['ms'])
...
def srlconll(y_true, y_pred, print_tex=True):
    '''
    ...
    return precrecf1(E['ok'], E['op'], E['ms'])

if __name__ == '__main__':
    y_pred = [1, 1, 1, 1]
    y_true = [0, 1, 2, 1]

    srlconll(y_true, y_pred);
